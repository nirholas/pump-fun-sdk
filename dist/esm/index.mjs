...
var ONE_BILLION_SUPPLY;
var init_fees = __esm({
  "src/fees.ts"() {
    "use strict";
    init_bondingCurve();
    ONE_BILLION_SUPPLY = new BN(1e15);
  }
});
...
function getStaticRandomFeeRecipient() {
  ...
  return new PublicKey2(recipient);
}
...
var LAMPORTS_PER_SOL = new BN3(1e9);
var ONE_TOKEN = new BN3(1e6);
function spotPrice(bondingCurve) {
  if (bondingCurve.virtualTokenReserves.isZero()) return new BN3(0);
...
function calculateBuyPriceImpact({
  ...
  const priceAfter = newVirtualTokenReserves.isZero() ? new BN3(0) : newVirtualSolReserves.mul(LAMPORTS_PER_SOL).div(newVirtualTokenReserves);
...
function getGraduationProgress(global, bondingCurve) {
  ...
  if (initialReal.isZero()) {
    return {
      ...
      isGraduated: false,
      tokensRemaining: new BN3(0),
      tokensTotal: new BN3(0),
      solAccumulated: new BN3(0)
    };
  }
...
function totalUnclaimedTokens(globalVolumeAccumulator, userVolumeAccumulator, currentTimestamp = Date.now() / 1e3) {
  ...
  const currentTimestampBn = new BN4(currentTimestamp);
...
var OnlinePumpSdk = class {
  ...
  async fetchUserVolumeAccumulatorTotalStats(user) {
    const userVolumeAccumulator = await this.fetchUserVolumeAccumulator(
      ...
      totalUnclaimedTokens: new BN5(0),
      totalClaimedTokens: new BN5(0),
      currentSolVolume: new BN5(0)
    };
  ...
  async getCreatorVaultBalance(creator) {
    ...
    return new BN5(accountInfo.lamports - rentExemptionLamports);
  }
  ...
  async adminUpdateTokenIncentives(startTime, endTime, dayNumber, tokenSupplyPerDay, secondsInADay = new BN5(86400), mint = PUMP_TOKEN_MINT, tokenProgram = TOKEN_2022_PROGRAM_ID) {
  ...
  async adminUpdateTokenIncentivesBothPrograms(startTime, endTime, dayNumber, tokenSupplyPerDay, secondsInADay = new BN5(86400), mint = PUMP_TOKEN_MINT, tokenProgram = TOKEN_2022_PROGRAM_ID) {
    return [
  ...
  async getMinimumDistributableFee(mint, simulationSigner = new PublicKey3(
    ...
    let minimumDistributableFee = {
      minimumRequired: new BN5(0),
      distributableFees: new BN5(0),
      canDistribute: false
    };
    if (!result.value.err) {
  ...
  async fetchBondingCurveSummary(mint) {
    const mintPk = new PublicKey3(mint);
  ...
  async sellAllInstructions({
    ...
    const amount = new BN5(accountInfo.data.subarray(64, 72), "le");
    if (amount.isZero()) {
      return [
        createCloseAccountInstruction(
          associatedUser,
  ...
  async isGraduated(mint) {
    const poolAddress = canonicalPumpPoolPda(new PublicKey3(mint));
   ...
   */
  async getTokenBalance(mint, user, tokenProgram = TOKEN_PROGRAM_ID) {
    const ata = getAssociatedTokenAddressSync(mint, user, true, tokenProgram);
    const accountInfo = await this.connection.getAccountInfo(ata);
    if (!accountInfo) return new BN5(0);
    return new BN5(accountInfo.data.subarray(64, 72), "le");
  }
};
...
var PumpSdk = class {
  ...
  async buyInstruction({
    ...
    return await this.getBuyInstructionInternal({
      ...
      solAmount: solAmount.add(
        solAmount.mul(new BN6(Math.floor(slippage * 10))).div(new BN6(1e3))
      ),
      tokenProgram
    });
  }
  ...
  async ammBuyInstruction({
    ...
    const userBaseAta = getAssociatedTokenAddressSync2(
      mint,
      user,
      true,
      TOKEN_2022_PROGRAM_ID2
    );
    const userQuoteAta = getAssociatedTokenAddressSync2(
      NATIVE_MINT2,
      user,
      true,
      TOKEN_PROGRAM_ID2
...
var feeProgramGlobalPda = () => {
  return pumpFeePda([Buffer2.from("fee-program-global")]);
};
...
var AMM_FEE_CONFIG_PDA2 = pumpFeePda([
  Buffer2.from("fee_config"),
  PUMP_AMM_PROGRAM_ID.toBuffer()
]);
var AMM_GLOBAL_CONFIG_PDA = pumpAmmPda([
  Buffer2.from("global_config")
]);

// src/index.ts
init_fees();
export {
  AMM_FEE_CONFIG_PDA2 as AMM_FEE_CONFIG_PDA,
  AMM_GLOBAL_CONFIG_PDA,
  AMM_GLOBAL_PDA,
  ...
  ONE_BILLION_SUPPLY,
  OnlinePumpSdk,
  PUMP_AMM_EVENT_AUTHORITY_PDA2 as PUMP_AMM_EVENT_AUTHORITY_PDA,
  PUMP_AMM_PROGRAM_ID,
